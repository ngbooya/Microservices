...
def getRecentArticle(number):
	if request.method=='GET':
		cur = get_db().cursor()
		res = cur.execute('''SELECT * FROM articles ORDER BY date DESC LIMIT ''' + str(number) + ";")
		data = res.fetchall()
		return jsonify(data), 200
...
def getRecentArticleMetaData(number):
	if request.method=='GET':
		cur = get_db().cursor()
		res = cur.execute('''SELECT title, body, user_id, date, article_id FROM articles ORDER BY date DESC LIMIT ''' + str(number) + ";")
		data = res.fetchall()
		return jsonify(data), 200
...
def createUser():
	...
	try:
		...
		return jsonify({'error':'no email was passed'}), 400
	return jsonify({}),201

#CHANGE THE PASSWORD OF A USER
@app.route("/users/changepassword/<id>",methods=['POST'])
@auth_required
def changePassword(id):
	...
	try:
		...
		cur.execute('UPDATE users SET password="' + content['password'] + '" WHERE user_id=' + id + ';')
		conn.commit()
		conn.close()
	except:
